...
if (args.length < 3) {
    console.error('Uso: node index.js b|w <ARQUIVO DE BACKUP XML> <DIRETÓRIO DE SAÍDA> [m|s] [paragraph-fix] [ignore-links] [ignore-comments]');
    process.exit(1);
}
...
const ignoreLinks = args.includes('ignore-links');
const ignoreComments = args.includes('ignore-comments');
...
// Função para configurar o Turndown Service
